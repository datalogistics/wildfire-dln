...
public class ExternalBtLifecycle implements Lifecycle {

    private final Context pluginContext;
    private final Collection<MapComponent> overlays;
    private MapView mapView;

    private final static String TAG = ExternalBtLifecycle.class.getSimpleName();

    public ExternalBtLifecycle(Context ctx) {
    ...
    public void onCreate(final Activity arg0, final transapps.mapi.MapView arg1) {
        ...
        ExternalBtLifecycle.this.overlays.add(new ExternalBtMapComponent());

        // create components
        Iterator<MapComponent> iter = ExternalBtLifecycle.this.overlays.iterator();
        MapComponent c;
        while(iter.hasNext()) {
            c = iter.next();
            try {
                c.onCreate(ExternalBtLifecycle.this.pluginContext, 
                           arg0.getIntent(), 
                           ExternalBtLifecycle.this.mapView);
            } catch(Exception e) {
