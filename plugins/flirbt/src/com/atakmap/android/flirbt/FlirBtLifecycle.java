...
public class FlirBtLifecycle implements Lifecycle {

    private final Context pluginContext;
    private final Collection<MapComponent> overlays;
    private MapView mapView;

    private final static String TAG = FlirBtLifecycle.class.getSimpleName();

    public FlirBtLifecycle(Context ctx) {
    ...
    public void onCreate(final Activity arg0, final transapps.mapi.MapView arg1) {
        ...
        FlirBtLifecycle.this.overlays.add(new FlirBtMapComponent());

        // create components
        Iterator<MapComponent> iter = FlirBtLifecycle.this.overlays.iterator();
        MapComponent c;
        while(iter.hasNext()) {
            c = iter.next();
            try {
                c.onCreate(FlirBtLifecycle.this.pluginContext, 
                           arg0.getIntent(), 
                           FlirBtLifecycle.this.mapView);
            } catch(Exception e) {
