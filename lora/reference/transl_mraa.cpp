...
static const int SPI_BUS = 0; // yep, needs to be this. not 1.
static const int SPI_FREQ = 500000; 
//static const int SPI_FREQ = 65536; // max for RPi -- no effect
...
bool sx1272 = false; // hat uses sx1276
...
// GPIO pins -- failed
//int dio0  = 4; // WiringPi pin 7, physical pin 7, GPIO pin 4
//int ssPin = 25; // WiringPi pin 6, physical pin 22. GPIO pin 25
//int RST   = 17; // WiringPi pin 0, physical pin 11, GPIO pin 17

// UP pinout -- failed 
//int dio0  = 2; // WiringPi pin 7, physical pin 7, GPIO pin 4, UP pin 2
//int ssPin = 20; // WiringPi pin 6, physical pin 22. GPIO pin 25, UP pin 20
//int RST   = 3; // WiringPi pin 0, physical pin 11, GPIO pin 17, UP pin 3
...
byte readReg(byte addr)
{
    ...
    //rxptr = mraa_spi_write_buf_word(spi, (uint16_t*)spibuf, 2);

    //result = mraa_spi_write(spi,spibuf[0]);
    ...
    printf("\t%x %x => %x %x\n",spibuf[0],spibuf[1],rxbuf[0],rxbuf[1]);
...
void writeReg(byte addr, byte value)
{
    ...
    //uint16_t dabuf = (spibuf[0]<<8) + spibuf[1];
    //uint8_t right_half = 0;

    //printf("%x %x %x\n",spibuf[0],spibuf[1],dabuf);

    //wiringPiSPIDataRW(CHANNEL, spibuf, 2);


    //mraa_spi_write_buf(spi,spibuf, 2);
    mraa_spi_transfer_buf(spi, spibuf,(uint8_t*)rxbuf, 2);


    //printf("\t%x %x => %x %x\n",spibuf[0],spibuf[1],rxbuf[0],rxbuf[1]);

    //mraa_spi_write_word(spi, dabuf);
    ...
    //printf("\t%x %x %x %x\n",spibuf[0],spibuf[1],dabuf,right_half);
}

void reg_sweep(){
    uint16_t kmin = 0;
    uint16_t kmax = 0x42;
    uint16_t ksup = 0x42 + 1;
    uint16_t K;

    for (K=kmin; K < ksup; K += 1){
        //printf("(%x,%x)",K,readReg(K));
        printf("%x",readReg(K));
...
void SetupLoRa()
{
    ...
    if (sx1272) {
        ...
        writeReg(REG_MODEM_CONFIG2,(sf<<4));
    // how large is the word written to SPI? 2 hex = | 0x04);
    } else {
        if (sf == SF11 || sf == SF12) {
...
boolean receive(char *payload) {
    ...
    if((irqflags & 0x20) == 0x20)
    {
        ...
        printf("\ncurrAddr= %x receivedCount=%x receivedbytes=%x \n",
            currentAddr,receivedCount,receivedbytes);
...
void receivepacket() {
    ...
    {
        if(receive(message)) {
            //byte value = readReg(REG_PKT_SNR_VALUE); // prev
...
int main (int argc, char *argv[]) {
    ...
    //spi = mraa_spi_init_raw(SPI_BUS,1); // nope
    ...
    // how large is the word written to SPI? 2 hex = 
    //status = mraa_spi_bit_per_word(spi, 8); // if spi_buf has len 2
    status = mraa_spi_bit_per_word(spi, 8); // so spi_buf has len 1
    //status = mraa_spi_bit_per_word(spi, 16); // so spi_buf has len 1
    if (status != MRAA_SUCCESS) {
    ...
    if (!strcmp("sender", argv[1])) {
        ...
        if (argc > 2){
            ...
            printf("%s %s %d\n",hello, argv[2],sizeof(hello));
         }

        while(1) {
            txlora(hello, strlen((char *)hello));
            sleep(5); // in unsigned int seconds
        }
    } else {
